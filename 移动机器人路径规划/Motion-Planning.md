...
- 点到点的路径规划是指已知两点的情况下，在两点间寻找最优（如代价最小、路径最短、时间最短）路径。点到点包括对全局进行规划和对局部进行规划。全局路径规划注重寻求最优解，局部路径规划注重避障。
...
![title](https://raw.githubusercontent.com/XQLong/Logging/master/img/2019/07/16/1563286439817-1563286439823.png)

</div>

n为当前计算节点；start为起始节点；goal为目标节点；x,y分别是节点的行列号；p,q,w是权值。

### 3.3.1 距离信息表达

为避免领域中代价相同而造成的路径不确定性和抖动现象，将坐标差加权后再求和。通过列举不同加权比例验算搜索效率，发现p=6，q=10时具有较高的搜索效率。

在改进的启发函数中采用分区加权，则是为了起到一定的方向引导作用。
...
如图所示，以`X~1~=Y~1~`为分割线，当`X~1~>Y~1~`时，当前点到终点在X方向上的距离差大于Y方向上的距离差，因为选择计算节点是优先选择代价小的节点，所以启发式采用`6*X~1~+10*Y~1~`（大的权重放在小的距离差上）会引导搜索方向向`Y~1~`小的方向前进；同理分割线右上方，当前点到终点在X方向上的距离差小于Y方向上的距离差，启发式采用`10*X~1~+6*Y~1~`（大的权重放在小的距离差上）会引导搜索方向向`X~1~`小的方向前进；进而引导搜索方向向目标点前进。
...
cross根据叉积计算获得，通常距离AB为常量；而CD是随着两个向量的夹角变化而变化的，当两者平行时CD达到最小值，当两者垂直时CD达到最大值，由于选择计算节点是优先选择代价小的节点，所以加入角度信息后悔诱导A*算法偏向分布在起始点到目标点连线附近的节点，从而缩小搜索范围，提高搜索效率。
...
如图所示分别为传统`A*`算法，改进障碍搜索后的`A*`算法，改进启发函数后的`A*`算法三种算法在相同的地图模型中进行路径搜索得到的结果。
可以看到，在改进障碍搜索后障碍物与路径最小距离变大，增大了机器人移动的安全距离；而在改进启发函数后，向目标点的导向能力更强，大大减小了`A*`算法的搜索范围，提高了搜索效率。
...
可以看到实验数据与定性分析一致，改进后的算法在保证路径的安全性的同时，而且大大减小了`A*`算法的搜索范围，提高了搜索效率。
...
[动画展示各种路径搜素算法](http://www.webhek.com/post/pathfinding.html)