...
如图所示分别为传统A*算法，改进障碍搜索后的A*算法，改进启发函数后的A*算法三种算法在相同的地图模型中进行路径搜索得到的结果。
可以看到，在改进障碍搜索后障碍物与路径最小距离变大，增大了机器人移动的安全距离；而在改进启发函数后，向目标点的导向能力更强，大大减小了A*算法的搜索范围，提高了搜索效率。
...
可以看到实验数据与定性分析一致，改进后的算法在保证路径的安全性的同时，而且大大减小了A*算法的搜索范围，提高了搜索效率。
