...
如图所示，以X~1~=Y~1~为分割线，当X~1~>Y~1~时，当前点到终点在X方向上的距离差大于Y方向上的距离差，因为选择计算节点是优先选择代价小的节点，所以启发式采用`6*X~1~+10*Y~1~（大的权重放在小的距离差上）会引导搜索方向向Y~1~小的方向前进；同理分割线右上方，当前点到终点在X方向上的距离差小于Y方向上的距离差，启发式采用10*X~1~+6*Y~1~（大的权重放在小的距离差上）会引导搜索方向向X~1~小的方向前进；进而引导搜索方向向目标点前进。
