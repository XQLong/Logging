...
如图所示是最佳优先算法的伪代码（此代码仅仅体现了搜寻过程，没有存储节点间父子关系来追溯路径）。最佳优先搜索算法路径规划，主要使用了一个优先队列作开启列表来存储搜寻过程中待检索的节点，并保证每次从权值最小节点的开始搜索；使用关闭列表存储不需要再次检索的列表；使用追溯表存储父子节点的关系，用于追溯生成路径。由于与A*算法流程出启发函数
